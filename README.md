...
```
Эта функция показала хорошее распределение слов по хэшу, поэтому я остановился на ней. После замены функции хэша время поиска увеличилось. По новому профилю perf, определил, что теперь функция хэша тратит больше всего времени (см. рис. 12). Это связано с тем, что обычная версия этой функции содержит два двойных цикла, в которых обрабатываются 64 битные части слова в формате __m128i. 
